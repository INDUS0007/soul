...
class _SupportGroupsPageState extends State<SupportGroupsPage> {
  ...
  Future<void> _toggleMembership(SupportGroupItem group) async {
    final action =
        group.isJoined ? SupportGroupAction.leave : SupportGroupAction.join;
    // Capture context before async gap to avoid using BuildContext after await
    ...
    try {
      ...
      showSuccessSnackBar(
        useContext,
        updated.isJoined ? 'Joined ${updated.name}' : 'Left ${updated.name}',
      );
    } on ApiClientException catch (error) {
      if (!mounted) return;
      showErrorSnackBar(useContext, error.message);
    } catch (error) {
      if (!mounted) return;
      showErrorSnackBar(useContext, 'Unable to update membership. Please try again. ($error)');
    }
  }

  // ------------------------------
  //   SOCIAL MEDIA LINK OPENER
  // ------------------------------
  Future<void> _openUrl(String url) async {
    // URL launching functionality not available (url_launcher package not in dependencies)
    showErrorSnackBar(context, "Link opening feature not available");
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Row(
              children: [
                Icon(Icons.link_rounded, color: Color(0xFF8B5FBF), size: 28),
                SizedBox(width: 12),
                Text(
